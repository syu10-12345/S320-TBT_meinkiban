...
class TFTand9axis_sensor {
  private:
    ...
    typedef struct { Vector a, b; } Line;
    ...
    void topTriinitializer() {
        ...
        topTri[1].x = (float)(size / 2); topTri[1].y = gy_dis - (float)size;
        topTri[2].x = (float)(-size / 2); topTri[2].y = gy_dis - (float)size;
    }
    ...
    int DrawFixedGUI(int lineSpacing = 5) {
        ...
        for (int i = 1; i <= 10; i++) {
            if (i % 5 == 0) DrawLine(-15, lineSpacing * i, 15, lineSpacing * i, scaleColor);
            else DrawLine(-10, lineSpacing * i, 10, lineSpacing * i, scaleColor);
        }
        for (int i = 1; i <= 10; i++) {
            if (i % 5 == 0) DrawLine(-15, -lineSpacing * i, 15, -lineSpacing * i, scaleColor);
            else DrawLine(-10, -lineSpacing * i, 10, -lineSpacing * i, scaleColor);
    ...
    void updataLegacy(float torim, double IAS, double rpm, double ALT) {
        ...
        Rz_tri[0][0] = cosf(triAngle); Rz_tri[0][1] = -sinf(triAngle);
        Rz_tri[1][0] = sinf(triAngle); Rz_tri[1][1] = cosf(triAngle);
        ...
        memset(Rx, 0, sizeof(Rx));
        Rx[0][0] = Rx[3][3] = 1.f;
        Rx[1][1] = cosf(-phi); Rx[1][2] = sinf(-phi);
        Rx[2][1] = -sinf(-phi); Rx[2][2] = cosf(-phi);
    ...
    bool shouldRedrawChar(float torim, double IAS, double rpm, double ALT) {
        ...
        if (first) {
            first = false;
            lastMs = now;
            prevTorim = torim; prevIAS = IAS; prevRpm = rpm; prevAlt = ALT;
            return true;
        }
  ...
  public:
    ...
    void begin() {
        ...
        if (!spritesOk) {
            if (fixedSprite.created()) fixedSprite.deleteSprite();
            if (dynamicSprite.created()) dynamicSprite.deleteSprite();
        }
